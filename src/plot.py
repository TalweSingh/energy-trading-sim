...
class SimulationVisualizer:
    ...
    def __init__(
        self, metrics: Dict[str, Any], order_history: pd.DataFrame, max_time_to_fill=600
    ):
        ...
        if not order_history.empty:
            ...
            for idx, row in self.orders.iterrows():
                if row["order_id"] in fill_times:
                    ...
                    self.orders.at[idx, "time_to_fill"] = (
                        fill_time - row["submission_time"]
                    ).total_seconds() / 60
    ...
    def plot_buy_orders(self, strategy_id=None, title=None):
                ...
                go.Scatter(
                    ...
                    customdata=np.column_stack(
                        (
                            ...
                            filled_buys["submission_time"].dt.strftime(
                                "%Y-%m-%d %H:%M"
                            ),
                ...
                go.Scatter(
                    ...
                    customdata=np.column_stack(
                        (
                            ...
                            unfilled_buys["submission_time"].dt.strftime(
                                "%Y-%m-%d %H:%M"
                            ),
        ...
        fig.update_traces(
            marker=dict(size=12), opacity=1.0, hovertemplate=hovertemplate
        )
        ...
        fig.update_traces(
            marker=dict(
                ...
                cmin=0,
                cmax=(
                    self.max_time_to_fill if hasattr(self, "max_time_to_fill") else 600
                ),
            )
        )
                ...
                go.Scatter(
                    ...
                    customdata=np.column_stack(
                        (
                            ...
                            filled_sells["submission_time"].dt.strftime(
                                "%Y-%m-%d %H:%M"
                            ),
                ...
                go.Scatter(
                    ...
                    customdata=np.column_stack(
                        (
                            ...
                            unfilled_sells["submission_time"].dt.strftime(
                                "%Y-%m-%d %H:%M"
                            ),
    ...
    def create_metrics_table(self):
        ...
        if "fill_rate" in self.metrics:
            for strategy_id, data in self.metrics["fill_rate"].items():
                ...
                if (
                    "execution_prices" in self.metrics
                    and strategy_id in self.metrics["execution_prices"]
                ):
                ...
                # Add time to fill
                if (
                    "time_to_fill" in self.metrics
                    and strategy_id in self.metrics["time_to_fill"]
                ):
                ...
                # Add volume
                if (
                    "contract_volume" in self.metrics
                    and strategy_id in self.metrics["contract_volume"]
                ):
                ...
                # Add buy cost
                if (
                    "buy_cost" in self.metrics
                    and strategy_id in self.metrics["buy_cost"]
                ):
                ...
                if (
                    "volume_execution_rate" in self.metrics
                    and strategy_id in self.metrics["volume_execution_rate"]
                ):
                    vol_exec_data = self.metrics["volume_execution_rate"][strategy_id]
                    if "intended_volume" in vol_exec_data:
                        row["Intended Volume"] = (
                            f"{vol_exec_data['intended_volume']:.2f}"
                        )
        ...
        fig = go.Figure(
            data=[
                go.Table(
                    ...
                    cells=dict(
                        values=[
                            [row[col] for row in table_data] for col in all_columns
                        ],
                        align="left",
                        font=dict(size=10),
                    ),
        ...
        fig.update_layout(
            title="Strategy Performance Metrics", height=130 + 30 * len(table_data)
        )
