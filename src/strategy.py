...
class Strategy(ABC):
    ...
    def __init__(
        ...
        load_profile: Optional[pd.DataFrame] = None,
    ):
    ...
    def process_results(self, results: Dict[str, List]) -> None:
        ...
        self.active_orders = {
            order.order_id: order for order in results.get("active_orders", [])
        }

    def create_order(
        self, price: float, quantity: float, contract_time: datetime.datetime, side: str
    ) -> Any:
