    ...
    for day_offset in range(num_days):
        ...
        for hour in range(24):
            ...
            delivery_time = datetime.datetime.combine(
                delivery_date.date(), datetime.time(hour, 0)
            )
            ...
            overnight_points = np.random.exponential(
                total_minutes / 20, overnight_count
            )
            ...
            remaining_points = np.random.power(power_param, remaining_count) * (
                total_minutes * 0.3
            )
            ...
            if trades_for_contract:
                ...
                df_contract["price_volume"] = (
                    df_contract["price"] * df_contract["volume"]
                )
                overall_vwap = (
                    df_contract["price_volume"].sum() / df_contract["volume"].sum()
                )
