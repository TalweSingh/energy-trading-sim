...
class SimulationMetrics:
    ...
    def fill_rate(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy_id, strategy_orders in self.order_by_strategy.items():
            ...
            submitted = strategy_orders[
                strategy_orders["event_type"] == "submitted"
            ].shape[0]
    ...
    def contract_volume(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy_id, strategy_orders in self.order_by_strategy.items():
            ...
            volume_by_contract = filled_orders.groupby("contract_time")[
                "quantity"
            ].sum()
    ...
    def execution_prices(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy_id, strategy_orders in self.order_by_strategy.items():
            ...
            ).sum() / filled_orders["quantity"].sum()
            ...
            if not buy_orders.empty:
                buy_vwap = (
                    buy_orders["price"] * buy_orders["quantity"]
                ).sum() / buy_orders["quantity"].sum()
            ...
            if not sell_orders.empty:
                sell_vwap = (
                    sell_orders["price"] * sell_orders["quantity"]
                ).sum() / sell_orders["quantity"].sum()
    ...
    def volume_execution_rate(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy_id, strategy_orders in self.order_by_strategy.items():
            ...
            submitted_orders = strategy_orders[
                strategy_orders["event_type"] == "submitted"
            ]
            ...
            execution_rate = (
                executed_volume / intended_volume if intended_volume > 0 else 0
            )
            ...
            for side in ["buy", "sell"]:
                ...
                if not side_submitted.empty:
                    ...
                    side_executed = (
                        side_filled["quantity"].sum() if not side_filled.empty else 0
                    )
                    side_rate = (
                        side_executed / side_intended if side_intended > 0 else 0
                    )
